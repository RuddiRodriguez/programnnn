...
xdata= load(sprintf('%s/complex/intervals_modos2.txt',working_directory_path));
h=0;
contournr=2
d=0
while exist(sprintf('%s/complex/flucts_modos%i.txt',working_directory_path,contournr)) == 2
   h=h+1;
   ydata= load(sprintf('%s/complex/flucts_modos%i.txt',working_directory_path,contournr));
   
   
   xdata1=xdata(1:160,1)
 ydata1=ydata(1:160,1)
...
plot(log10(xdata),log10(ydata),'o',log10(xdata1),f,'k')
  ...
  axis square
   ...
   save(sprintf('D:/DFA/coef%i.txt',filenamesaved),'alfa','-ascii');

...
% xdata=xdata'
% ydata=ydata'
options = statset('MaxIter' , 56000000, 'DerivStep',1e-12 );
options.TolFun=1e-80
options.TolX=1e-80
options.MaxFunEvals=600000000

%TolFun',200,'TolX',200,'MaxFunEvals',200
...
beta0 =[6e-8 6e-20 12 1e9 0.1];
modelFunw = @(b,xdata) 1e7*model_spectrum_hy(b,xdata,rmean,h,ydata);

%mdl= fitnlm (xdata,ydata,modelFunw,beta0)
  [bFitw,rw,Jw,COVBw] = nlinfit(xdata,ydata,modelFunw, beta0, options);
 %bFitw = lsqcurvefit(modelFunw, beta0,xdata,ydata,[],[])
...
msgbox(sprintf('Sigma = %g \nKappa = %g \nepsilon = %g \nb = %g \nh = %g \nc = %g \nradio = %g \nsse = %g \nrsquare = %g \nadjrsquare = %g',sigma,kappa,epsilon,b,h,c,rmean,gof.sse,gof.rsquare,gof.adjrsquare),'Fitting Results','none','non-modal')

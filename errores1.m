...
g=0;
 ...
 sumarhotheta(j)=sum((((abs(factor4-factor5)).*abs((factor7-factor6)))./(4.*pi)).*factor11);
 sigmaR2(j)= sumatheta(j)+sumarho(j)+sumarhotheta(j);
 sigmaR2;
...
%  %Calculo de sigma cuadrado an
  ...
  termino1suma1=(((sigmar0).^2).*((aa(j)./r0(j)).^2));
...
% end 
  ...
  sumaan2=sum((termino13.*(2.*(sigma0^2))).*(((1/(pi.*r0(j))).^2)).*(((termino10-termino11-termino12)./2).^2));
  ...
  sumaan3=2.*sum((((sigma0.^2)./(4.*pi)).*((((factor4-factor5))+(((factor7-factor6)).*termino13)))).*(((-(aa(j)./r0(j))).*(1/(pi.*r0(j)))).*termino14.*((factor4-factor5))));
  ...
  for s8=2:(length(rho)-1)
      termino16(s8)=(rho(s8+1)*m*(cos(m*thetar(s8+1))))-((rho(s8-1)*cos(m*thetar(s8-1)))/2);
                
  end
  
  factor1=(factor4-factor5)+((factor7-factor6).*(termino13));
  factor2=termino15.*((factor4-factor5)./2);
  
  sumaan4=2.*sum((((((sigma0).^2)./(4.*pi)).*termino13).*factor1.*((-aa(j)./(r0(j)))./(pi.*r0(j))).*((1./pi.*r0(j))).*(factor2-termino16)));
  ...
  sumaan5=2.*sum((((2.*(sigma0.^2)).*termino13).*(1/((pi./r0(j)).^2)).*termino18.*(factor4-factor5)).*((termino14.*((factor4-factor5)./2))-(termino11+termino12)));
  ...
  for s1=2:(length(rho)-1)
      termino9(s1)=((cos(m.*thetar(s1)))^2)./2;
                
  end
  termino1suma1=(((sigmar0).^2).*((bb(j)./r0(j)).^2));
  termino2suma2=(sigma0.^2).*((inv(pi*r0(j)))^2);
  termino3suma3=(factor4-factor5).^2;
  ...
  sumabn2=sum((termino13.*(2.*(sigma0^2))).*(((1/(pi.*r0(j))).^2)).*((termino10-termino11-termino12).^2));
  ...
  sumabn3=2.*sum((((sigma0.^2)./(4.*pi)).*(((factor4-factor5))+(((factor7-factor6)).*termino13))).*((-(bb(j)./r0(j))).*(1/(pi.*r0(j)))).*termino14.*((factor4-factor5)));
  ...
  sumabn4=2.*sum((((((sigma0).^2)./(4.*pi)).*termino13).*factor1.*((-bb(j)./(r0(j)))./(pi.*r0(j))).*((1./pi.*r0(j))).*(factor2-termino16)));
  ...
  sumabn5=2.*sum((((2.*(sigma0.^2)).*termino13).*(1/((pi./r0(j)).^2)).*termino18.*(factor4-factor5)).*((termino14.*((factor4-factor5)./2))-(termino11+termino12)));
  
  sigma2bn(j)=sumabn1+sumabn2+sumabn3+sumaan4+sumabn5;
...
%  cov=1cov(sigma2an,sigmaR2)
 cov1=cov(mean(cc.^2),mean(cc));
 cov2=cov(mean(cc.^2),mean(r0));
 cov3=cov(mean(cc),mean(r0));
 
 sigma2cnmodulo2=(1/(j^2)).*sum((sigma2cnsq).^2);%cuadrado
 sigmacnlineal=(1/(j^2)).*sum((sigma2cn).^2);%lineal
 sigmaR2mean=(1/(j^2)).*sum(sigmaR2);
%  sigma2q(m)=(sigmaR2.*(m^2))./(mean(r0)^4);
 
 %calculo de sigma2u
 
  termino20=(((pi/2).*((mean(r0))^3))^2).*sigma2cnmodulo2;
  termino21=(((pi/2).*((mean(r0))^3))^2).*((2.*mean(cc)).^2).*sigmacnlineal;
  termino22=(3.*(((mean(cc.^2))-((mean(cc)).^2))).*(pi/2).*((mean(r0)).^2))^2.*(sigmaR2mean);
  termino23=4*mean(cc)*(((pi/2).*((mean(r0))^3))^2).*cov1;
  termino24=(mean(r0)).^3.*(mean(cc.^2)-mean(cc));
  termino25=((3*(pi^2))/2).*mean(r0).*cov2;
  termino26=mean(cc).*(pi^2).*(mean(r0)^3).*3.*((mean(cc.^2)-mean(cc)));
  termino27=mean(r0)^2.*cov3;
...
% save(sprintf('%s/results/errores1.txt',working_directory_path),'sigma2q','-ascii')
%end
% figure(7)

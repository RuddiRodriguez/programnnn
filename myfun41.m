...
% p1(i)=P((q+1));
%x(4)=500
deno=((l-1).*(l+2).*((l.*(l+1))+x(4)-(4.*x(5))+(2.*((x(5)).^2))));
P1(i)=p./deno;
% a=((1.38e-23*298)/(x(1)))*symsum(P/((l-1)*(l+2)*((l*(l+1))+x(2))),l,q,15)

end
% x(1)=u ,x(2)=kappa, x(3)=potencial, x(4)=excessarea, x(5)=curvatura espontanea  
% x(6)=h, x(7)=epsilon
%x(6)=4e-9
kbt=1.38e-23*296;
alfap=(9.*x(1).*kbt)/(16.*pi.*x(2));
kappab=x(2)+(alfap.*(q.^-2))+(x(3).*(q.^-4))
keff=(3.*kappab./q)./(1+(6.*(((rmean.^1)./((x(6))^1)).*(kappab.*(q.^1)./x(7)))))         
factor=sum(P1);
poli=sum(p1);
factor1(h)=sum(P1);
kappa(h)=((1.38e-23*296)./(flucta(h))).*factor;
amplitud(h) =((1.38e-23.*298)./(keff)).*factor;
Res(h) =flucta(h)-((1.38e-23.*298)./(keff)).*factor;


end
sigma =x(4)*x(2)/(rmean^2);
q=v1:v;
...
text(10,1e-8,sprintf('kappa=%2.3e\n sigmaR=%2.3e\n CE=%2.3e\n sigma=%2.3e\n u=%2.3e\n ga=%2.3e\n  h=%2.3e\n epsilon=%2.3e',x(2),x(4),x(5),sigma,x(1),x(3),x(6),x(7)))
% hold on

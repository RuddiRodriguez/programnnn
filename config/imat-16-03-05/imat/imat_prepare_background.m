function [background] = imat_prepare_background(pic,bgvalue,sgn,init)
%====================================================
            ...
            for i=-1:1
                for j=-1:1
                    if chkpoints(n,1)+i>0 && chkpoints(n,2)+j>0 && chkpoints(n,1)+i<=P1 && chkpoints(n,2)+j<=P2
                        if sgn==0
                            if pic(chkpoints(n,1)+i,chkpoints(n,2)+j)<=bgvalue  && background(chkpoints(n,1)+i,chkpoints(n,2)+j)==0

...
diffrcos = zeros(mempospollength,1);
...
for i = 1:mempospollength
    tmp = difftheta(i) ^ 2 + (diffr(i) / mempospol(i,1)) ^ 2 + (difftheta(i) * diffr(i)) / mempospol(i);
    tmp2 = tmp2 + tmp;
end
sigma_r0_squared(j) = tmp2 * (2 * (sigma_0 ^ 2) / ((4 * pi) ^ 2))   ;

% sigma_r0_squared(j) = (2 * sigma_0 ^ 2 / (4 * pi) ^ 2) * ((difftheta' * difftheta) + diffr .^ 2 / (mempospol(:,1) .^ 2)  + difftheta .* diffr / mempospol(:,1));

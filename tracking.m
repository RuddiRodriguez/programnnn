...
contourclosedindex = zeros(contournr,1); % this will hold an index of whether a membrane was closed or not
...
tempp=zeros(parameterStruct.maxnrofpoints,2);
...
F(contournr) = struct('cdata',[],'colormap',[]);
...
% open image for tracking
    if filenumberdigits == 4
        camino=sprintf('%s/images/%s%04d.tif',working_directory_path,filename,j);
    image = imread(camino); % creates nxn-matrix of grayscale-image with intensity values
    end
    if filenumberdigits == 4
        camino=sprintf('%s/images/%s%04d.tif',working_directory_path,filename,j);
    image = imread(camino); % creates nxn-matrix of grayscale-image with intensity values
...
% display waitbar
h = awaitbar(0,'Tracking Contours, please wait...'); 
...
p = polylinear(pixpos(i,2)-parameterStruct.linfitparameter:pixpos(i,2)+parameterStruct.linfitparameter,...
            image(pixpos(i,1)+xdirection,pixpos(i,2)-parameterStruct.linfitparameter:pixpos(i,2)+parameterStruct.linfitparameter)); %calculate linear fit straight
...
p = polylinear(pixpos(i,1)-parameterStruct.linfitparameter:pixpos(i,1)+parameterStruct.linfitparameter,...
    transpose(image(pixpos(i,1)-parameterStruct.linfitparameter:pixpos(i,1)+parameterStruct.linfitparameter,pixpos(i,2)+ydirection))); %calculate linear fit straight
...
p = polylinear(linfitxvalues,linfitintensities);
...
for breakofpixel = parameterStruct.comparepixel:parameterStruct.comparepixel+parameterStruct.nrofcomparepixel 
    ...
    alpha = acos((a*transpose(b))/(norm(a)*norm(b)));
        ...
        finished = 1;
        contourclosed = 1;
        % write to the variable 'contourclosedindex' whether the contour was closed
        contourclosedindex(j) = 1;
        break;
        end
    end
    end
...
% remove the duplicately tacked membrane positions
...
save(camino,'xymempos','-ascii'); % save the contour
...
% one contour to the next as may happen when the algorithm didn't track the
% contour correctly, but still recognized the contour as being closed;
% this will only be done once the contour was found to be corectly
% recognized once, i.e. once firstdetection equals 1
if firstdetection == 0
    ...
    if abs(newcenter - center) <= parameterStruct.maxcenterchange
        center = newcenter;
    end
end
...
% save information on tracking
if j == contournr
% make directory that will be holding the information on the specific
% project
mkdir(sprintf('%s/results',working_directory_path));

% save contourclosedindex
camino=sprintf('%s/results/contourclosedindex.txt',working_directory_path);
save(camino,'contourclosedindex','-ascii'); % save the contour

% this save the data displayed in the message box below into a log-file for
% later reference
fid = fopen(sprintf('%s/results/log_tracking.txt',working_directory_path),'w');
fprintf(fid,'Total Contours: %i \nClosed Contours: %i \nNot Closed Contours', ...
    contournr, ...
    length(find(contourclosedindex == 1)), ...
    length(find(contourclosedindex == 0)));
fclose('all');
...
msgbox(sprintf('Total Contours: %i \nClosed Contours: %i \nNot Closed Contours: %i', ...
    contournr, ...
    length(find(contourclosedindex == 1)), ...
    length(find(contourclosedindex == 0))),'Results','none','modal')
end

...
parameterStruct
mkdir(sprintf('%s/memposs_trunc',working_directory_path));
mkdir(sprintf('%s/contourscorregido_trunc',working_directory_path));
...
q=0;
n=first:last;
mempos=zeros(length(n),2);
Lo=zeros(1,length(savedcontours));
for j = savedcontours
% load contour
mempos1 = load(sprintf('%s/contours/contour%i.txt',working_directory_path,j));
mempos(:,1)=mempos1(first:last,1);
mempos(:,2)=mempos1(first:last,2);
...
camino=sprintf('%s/contourscorregido_trunc/contour%i.txt',working_directory_path,j);
...
save(sprintf('%s/contours/polarcontour_trunc%i.txt',working_directory_path,j),'mempospol','-ascii'); % save the contour
...
j;
save(sprintf('%s/memposs/mempos_trunc%i.txt',working_directory_path,j),'mempos','-ascii')
...
% save fluctuations
save(sprintf('%s/results/fluctuationsa_trunc.txt',working_directory_path),'fluctuationa','-ascii')

save(sprintf('%s/results/fluctuations_trunc.txt',working_directory_path),'fluctuation','-ascii')

save(sprintf('%s/results/wavenumbers_trunc.txt',working_directory_path),'wavenumbers','-ascii')

save(sprintf('%s/results/a_trunc.txt',working_directory_path),'a','-ascii')

save(sprintf('%s/results/b_trunc.txt',working_directory_path),'b','-ascii')

save(sprintf('%s/results/c_trunc.txt',working_directory_path),'c','-ascii')

save(sprintf('%s/results/r0_trunc.txt',working_directory_path),'r0','-ascii')
diametro=rmean*2*100

save(sprintf('%s/results/longitud_trunc.txt',working_directory_path),'Lo','-ascii'); % save the contour

  function void=errores (varargin)
clear
...
sigmacnlineal_1=zeros(parameterStruct.nmax,1);
%m=3%for m=1:3
...
% end 
  ...
  for s2=2:(length(rho)-1)
      ...
      termino11(s2)=rho(s2+1).*cos(m.*thetar(s2+1));  
      termino12(s2)=(rho(s2-1)*cos(m*thetar(s2-1)));%./2;
      termino13(s2)=1/(rho(s2));
      termino13(s2)=1/(rho(s2));
      termino14(s2)=cos(m*thetar(s2));
  end
...
%  sigmacne(m)=sigma2cn
waitbar(m/parameterStruct.nmax)
end

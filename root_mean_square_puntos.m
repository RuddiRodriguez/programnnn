...
N=1096;         % no. step to take
T= 0.3653;           %maximum time
h=T/N;          %time step
t1=(0:h:T); 
t1=t1';%
...
% ylabel('P(R)');
figure(3)
plot((StepTime(1:m)),(RMSD(1:m)),'-*')%,(StepTime(1:m)),(rmsd1(1:m)),'-o');
title('Mean Square Displacement of Brownain Motion');
xlabel('time step');
ylabel('MSD');
hold on
 save(sprintf('%s/results_puntos/rmsd%i.txt',working_directory_path,m1),'RMSD','-ascii');
